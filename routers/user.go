...
func UserRoutes(router *mux.Router) *mux.Router {
	...
	router.Handle("/user/updateemail", utls.Authorize(controllers.CheckAuthenticLogin(http.HandlerFunc(userController.UserUpdateEmail)))).Methods(http.MethodPost)
	router.Handle("/user/changepassword", utls.Authorize(controllers.CheckAuthenticLogin(http.HandlerFunc(userController.UserChangePassword)))).Methods(http.MethodPost)
